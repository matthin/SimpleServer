...
bool ss::Socket::disconnect() { ::close(handle); }

sockaddr_in ss::Socket::create_address(const std::string &host,
...
void ss::Socket::set_timeout(const long seconds, const long microseconds) {
  ...
  setsockopt(handle, SOL_SOCKET, SO_RCVTIMEO, (char *)&tv,
             sizeof(struct timeval));
}

void ss::Socket::set_timeout(const long seconds) { set_timeout(seconds, 0); }
